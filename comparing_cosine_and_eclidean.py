...
for embedding, title in zip(embeddings, titles):
    ...
    euclidean_distance(embedding, query_embedding)
    distance_score.append(euclidean_distance(embedding, query_embedding))

# creating data frame for score manipulation and viz
df = pd.DataFrame(columns=["Id", "Title", "Score", "E_distance"])

# loop to iterate over both lists simultaneously and append the items to df:
for ids, title, simi_score, dist in zip(ids, titles, similarity_score, distance_score):
    df = df.append({"Id": ids, "Title": title, "Score": simi_score, "E_distance": dist}, ignore_index=True)
